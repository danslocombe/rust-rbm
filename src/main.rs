...
fn try_main() -> Result<()> {
    ...
    let labels: Vec<Input> = raw_labels.iter()
        .map(|&x| {
    ...
    let sample_label_raw = from_stdin!("Enter a label to generate a sample from: ",
                                       usize,
                                       LABEL_START);
...
fn labels_from_file(filename: &String) -> Result<Vec<Label>> {
    ...
    f.split("\n")
        ...
        .map(|x| {
            x.parse::<Label>()
                .map_err(|why| RBMError(format!("Malformed label file, {}\n{}", filename, why)))
        })
        .collect()
}
...
fn read_file(s: &String) -> Result<String> {
    ...
    file.read_to_string(&mut s)
        .map_err(|why| RBMError(format!("couldn't read {}: {}", display, why.description())));
    Ok(s)
}
