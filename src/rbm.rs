...
impl RBM{
    ...
    pub fn epoch (&mut self, batch : &[Input], labels : &[Input]){
        ...
        let mut d_weights : Matrix<f32> = Matrix::zeros(self.labels + self.visible_size, self.hidden_size);
    ...
    pub fn sample(&self, clamped_labels : Input) -> Vector<f32> {
        let input : Vec<f32> = rand_vec(self.visible_size).iter().cloned()
                              .chain(clamped_labels.iter()
                                     .map(|x| *x as f32))
                              .collect();
        ...
        for _ in 1..self.sample_mcmc {
            ...
            v = Vector::from(self.prop_hidden(&h).iter().take(self.visible_size).cloned()
                              .chain(clamped_labels.iter()
                                     .map(|x| *x as f32))
                              .collect::<Vec<_>>());
        }
